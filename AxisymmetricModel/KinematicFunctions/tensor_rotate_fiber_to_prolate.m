function [ Trot ] = tensor_rotate_fiber_to_prolate( sps, cps, T )
...
Trot(:,:,3,3) = T(:,:,2,2).*sps.^2 + cps.*( 2*sps.*T(:,:,2,3) +  T(:,:,3,3).*cps );

Trot(:,:,2,1) = Trot(:,:,1,2) ;
Trot(:,:,3,1) = Trot(:,:,1,3);
Trot(:,:,3,2) = Trot(:,:,2,3);

% s2lambda = 2.*spsi.*clambda;
% c2lambda = clambda.^2 - spsi.^2;
% 
...
% Trot(:,:,1,1) = T(:,:,1,1);
% Trot(:,:,1,2) = T(:,:,1,2).*clambda + T(:,:,1,3).*spsi;
% Trot(:,:,1,3) = T(:,:,1,3).*clambda - T(:,:,1,2).*spsi;
% 
% Trot(:,:,2,2) = T(:,:,2,2).*clambda.^2 + T(:,:,2,3).*s2lambda  ...
%                     + T(:,:,3,3).*spsi.^2;
%                 
% Trot(:,:,2,1) = Trot(:,:,1,2);
% % 
% Trot(:,:,2,3) = T(:,:,2,3).*c2lambda + (T(:,:,3,3) - T(:,:,2,2)).*clambda.*spsi;
%                     
%                 
% Trot(:,:,3,1) = Trot(:,:,1,3);
% Trot(:,:,3,2) = Trot(:,:,2,3);
%                 
% Trot(:,:,3,3) = T(:,:,2,2).*spsi.^2 + clambda.*(T(:,:,3,3).*clambda - 2*spsi.*T(:,:,3,2));
